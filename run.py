...
class fun():
    ...
    def page_source(self):
        ua = random.choice(_user_agent)
        headers = {
            'User-Agent': ua
        }
        r = requests.get('http://www.luoo.net/music/' +
                         str(self.page_id), headers=headers)
        return r.content

    # 网页源码转为bs对象
    def turn_to_soup(self):
        soup = BeautifulSoup(
            self.page_source(),
            'html.parser',
            from_encoding='utf8')
    ...
    def page_song(self):
        try:
            songs = self.turn_to_soup().find_all(
    ...
    def run(self):
        threads = []
        task1 = threading.Thread(target=self.page_img())
        threads.append(task1)
        task2 = threading.Thread(target=self.page_song())
